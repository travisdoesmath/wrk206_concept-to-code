...
class Bee {
    ...
    move() {
        this.x += this.dx;
        this.y = Math.max(125, this.y + this.dy);
    ...
    setSpeed(dx, dy) {
        if (this.x + this.dx > canvas.width - 200 && dx > 0) {
            this.dx = -dx;
        } else if (this.x + this.dx < 125 && dx < 0) {
            this.dx = -dx;
        } else {
            this.dx = dx;
        }
        if (this.y + this.dy < 125 || this.y + this.dy > canvas.height) {
            this.dy = 0;
        } else {
            this.dy = dy;
        }
    ...
    accelerate(ddx, ddy) {
        this.dx += Math.max(-10, Math.min(10, ddx));
        this.dy += Math.max(-10, Math.min(10, ddy)) ;
        this.history.push({
            x: this.x,
            y: this.y,
            dx: this.dx,
...
let umbly = new Bee(0.5*canvas.width, 0.5*canvas.height);
let tick = 0;
...
function moveUp(e) {
    ...
    switch(e.key) {
        ...
        case "w":
        case "W":
            umbly.setSpeed(2 * (Math.random() - 0.5), -2 + -18 * Math.random());
            break;
        case "Left":
        case "ArrowLeft":
        case "a":
        case "A":
            umbly.setSpeed(-2 + -5 * Math.random(), 2 * (Math.random() - 0.5));
            break;
        case "Right":
        case "ArrowRight":
        case "d":
